...
class CompetitionController extends Controller
{
    ...
    public function copyData($competition)
    {
        $compModel = new Competition();
    ...
    public function refreshParticipantDetails($competition_part, $competition, $compModel)
    {
        ...
        if (is_null($compParticipant)) {
            ...
            ->select('KARATE_KA.*', DB::raw('KARATE_KA.TITLE'), DB::raw("IFNULL(KARATE_KA.NAME,'') AS NAME"), DB::raw("IFNULL(KARATE_KA.M_NAME,'') as M_NAME"), DB::raw("IFNULL(KARATE_KA.L_NAME,'') as L_NAME"),
            ...
            if ($karateKa) {
                ...
                $compParticipant->title = $karateKa->TITLE;
    ...
    public function refreshData($decrypted_comp_id, Request $request)
    {
        ...
        $pendingPartCompetitionSql = " SELECT A.*, K.TITLE, IFNULL(K.NAME,'') AS NAME, IFNULL(K.M_NAME,'') as M_NAME,IFNULL(K.L_NAME,'') as L_NAME
        ...
        foreach ($default_category as $compCategory) {
            ...
            $compBoutParticipants = Participant::where('competition_id', $compModel->id)->
                ...
                when($compCategory->group_rank, function ($query, $group_rank) {
                    if ($group_rank == 'WYO') {
                        $query->whereIn('rank_id', [0, 1, 2, 3]);
                    } elseif ($group_rank == 'GBP') {
                        $query->whereIn('rank_id', [4, 5, 6]);
                    } else {
                    }
                })->
                get();
...
;
    ...
    public function processFightData($result, $bouts_id)
    {
        // dd($bouts_id);
        ...
        if ($number_of_competitor > 0) {
            ...
            foreach ($arr as $item) {
                ...
                $AO = ! isset($data[$number_of_bout + $item]) ? (object) ['id' => 0] : $data[$number_of_bout + $item];
                $my_d = [
                    'Bout' => $item + 1,
                    'AKA' => $AKA,
                    'AO' => $AO,

...
class CompetitionBoutController extends Controller
{
    ...
    public function download_bout($decrypted_comp_id, $bout_id, $custom_bout_id)
    {
        ...
        $fileObject = new SplFileObject($outputFilePathNew);

        // Get the MIME type of the file using Laravel's Storage facade
        $mimeType = Storage::mimeType($outputFilePathNew);
        ...
        $headers = [
            ...
            'Content-Disposition' => 'attachment; filename="'.'Kumite_'.$decrypted_comp_id.'_'.$bout_id.'.pdf'.'"',
        ];

        $flag = FileHelper::delete_files($outputFilePath);

        $flag = FileHelper::delete_files($outputFilePathNew);
    ...
    public function results_report_download_external_coach_code($decrypted_comp_id, $external_coach_code, $download_type, Request $request)
    {
        ...
        if ($download_type == 'result') {
            $pdf = PDF::loadView('admin/bout/download_result', [
        ...
        } elseif ($download_type == 'certificate') {
    ...
    public function generate_certificate_for_coach($compModel, $coach, $participants)
    {
        ...
        foreach ($participants as $key => $participant) {
            [$fpdi, $outputFilePath] = $this->generate_certificate($compModel, $coach, $participant);
            $fpdi->Output($outputFilePath, 'F');
    ...
    public function generate_cards_for_coach($compModel, $coach, $participants)
    {
        ...
        foreach ($participants_array_chunk as $key => $participant) {
            [$fpdi, $outputFilePath] = $this->generate_cards($compModel, $coach, $participant, $key + 1);
            $fpdi->Output($outputFilePath, 'F');
            array_push($outputFileList, $outputFilePath);
        }
        // dd($outputFileList);
    ...
    public function get_image_url($part_comp_id)
    {
        $fileSql = "
            SELECT F.name 
